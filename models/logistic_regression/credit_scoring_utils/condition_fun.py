...
def rmcol_datetime_unique1(dat): # add more datatime types later
    ...
    # Commented out to avoid removing data from 1-row dataframes
...
def x_variable(dat, y, x):
    ...
    x_all = [el for el in list(dat) if el not in [y]]
    ...
    if x is None:
        ...
        if any([i in list(x_all) for i in x]) is False:
            ...
            x_notin_xall = [el for el in x if el not in x_all]
            if len(x_notin_xall) > 0:
                ...
                x = [el for el in x if el in x_all]
