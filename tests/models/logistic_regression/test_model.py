...
MOCK_RISK_ATTRIBUTES_LIST = [{'bank.account': 'nie',
                              'age.in.years': 33,
                              'savings': '1000 <= ... < 5000',
                              'purpose': 'biznes',
                              'property': 'samochód lub inne nie zawarte w oszczędnościach',
                              'credit.history': 'wszystkie pożyczki spłacone w terminie',
                              'duration.in.month': 12,
                              'present.employment.since': '... >= 7 lat',
                              'housing': 'wynajmowane'},
                             {'bank.account': 'tak',
                              'age.in.years': 25,
                              'savings': '... < 1000',
                              'purpose': 'przekwalifikowanie',
                              'property': 'nieruchomość',
                              'credit.history': 'wszystkie pożyczki spłacone w terminie',
                              'duration.in.month': 6,
                              'present.employment.since': '4 <= ... < 7 lat',
                              'housing': 'bezpłatne'},
                             {'bank.account': 'tak',
                              'age.in.years': 25,
                              'savings': '... < 1000',
...
MOCK_INVALID_RISK_ATTRIBUTES_LIST = [{'bank.account': 'nie',
                                      'age.in.years': 33,
                                      'savings': '1000 <= ... < 5000',
                                      'credit.purpose': 'biznes',
                                      'property': 'samochód lub inne nie zawarte w oszczędnościach',
                                      'credit.history': 'wszystkie pożyczki spłacone w terminie',
                                      'duration.in.month': 12,
                                      'present.employment.since': '... >= 7 lat',
...
class CalculateTest(TestCase):
    ...
    def test_calculate_returns_expected_data_structures(self):
        ...
        result = calculate(None, mock_data)["result"]
